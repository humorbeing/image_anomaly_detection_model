...
if __name__ == '__main__':
    ...
    model = densenet201(pretrained=True, fc_layer1=1024, fc_layer2=128,
                                     global_pooling_size=(7, 7))
    
    model = densenet169(pretrained=True, fc_layer1=1024, fc_layer2=128,
                                     global_pooling_size=(7, 7))
    
    model = densenet161(pretrained=False, fc_layer1=1024, fc_layer2=128,
                                     global_pooling_size=(7, 7))
    
    model = densenet121(pretrained=True, fc_layer1=1024, fc_layer2=128,
                                     global_pooling_size=(7, 7))
    
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.eval()
    input_feature = torch.rand(2,6,3,224,224).to(device)
    with torch.no_grad():
